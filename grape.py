...
def PI_Grow(ind_class, pop_size, bnf_grammar, min_init_depth, max_init_depth, codon_size, codon_consumption):
    ...
    for i in range(n_sets):
        ...
        for j in range(set_size):
            ...
            #Check if the individual was mapped correctly
            if remainders != ind.structure:
                print(ind.phenotype)
                raise Exception('error 1 in the mapping')
                
           #Check if the individual was mapped correctly
            if phenotype != ind.phenotype:
                print(ind.phenotype)
                raise Exception('error 2 in the mapping')
            
            #Check if the individual was mapped correctly
            if max(depths) != ind.depth:
                print(ind.phenotype)
                raise Exception('error 3 in the mapping')
